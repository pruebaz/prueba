...
int get_pid(){
	...
	printk("-> ID PROCESO %d\n", p_proc_actual->id);
	pid=p_proc_actual->id;

        return pid; /* devuelve el pid */
}
...
int sis_dormir (){
	...
	p_proc_actual->tiempo=segundos * TICK;
...
// Función para poner el proceso en bloqueado, reajustar listas , y realizar el cambio de contexto
static void bloquear (lista_BCPs * lista){ //se le pasa como parametros lista_dormidos
	int nivel;
...
static void ajustar_dormidos (){
	...
	BCP * proc_dormidos =  lista_dormidos.primero;
	while (proc_dormidos != NULL)
	{
		proc_dormidos->tiempo--;
		if (proc_dormidos->tiempo == 0)
		{
			desbloquear(proc_dormidos,&lista_dormidos);

		}
		proc_dormidos = proc_dormidos->siguiente;
	}
}
